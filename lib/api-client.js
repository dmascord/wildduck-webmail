...
module.exports = {
    users: {
        ...
        get(user, params, callback) {
            ...
            let args = { user: user.id, ...params };
            _exec('get', '/users/{user}', args, false, user.token, (err, obj) => {
                if (err && params && params.metaData && err.name === 'InputValidationError') {
                    return _exec('get', '/users/{user}', { user: user.id }, false, user.token, callback);
                }
        ...
        resolve(data, callback) {
            const { username, ip, sess } = data;
            _exec('get', '/users/resolve/{username}', { username, ip, sess }, false, config.api.accessToken, callback);
        }
    },
    ...
    authlog: {
        list(user, data, callback) {
            _exec(
                'get',
                '/users/{user}/authlog?next={next}&previous={previous}',
                { user: user.id, next: data.next || '', previous: data.previous || '' },
                false,
                user.token,
                callback
            );
    ...
    messages: {
        ...
        listWithParams(user, mailbox, params, callback) {
            params = params || {};
            _exec('get', '/users/{user}/mailboxes/{mailbox}/messages', { user: user.id, mailbox, ...params }, false, user.token, callback);
        },
        ...
        searchAction(user, payload, callback) {
            _exec('post', '/users/{user}/search', { user: user.id }, payload, user.token, callback);
        },
        ...
        raw(req, res, user, mailbox, message) {
            ...
            if (config.service.sso.http.enabled && !token) {
                token = config.api.accessToken;
            }
